...
public sealed class WindowController : IDisposable
{
    ...
    private OperationResult AlignTo(nint hwnd, PixelRect target, Stopwatch stopwatch)
    {
        ...
        // 最小化的窗口一律不对齐。
        //
        // 实测：最小化时 GetWindowRect 与 DWM 可见边框返回两个相差约 16000 像素的
        // 屏外矩形，据此算出的"不可见边框厚度"是垃圾值 —— 而下面那段会把它
        // **永久缓存**进 _borderPadding。多数窗口的缓存在正常状态时就已建好，
        // 但只要某个窗口的**第一次**对齐发生在最小化状态（崩溃恢复、收编最小化窗口），
        // 它此后每次对齐都会被摆到屏幕外。
        if (User32.IsIconic(hwnd))
        {
            return OperationResult.Fail("窗口已最小化，跳过对齐（此时算不出正确的边框厚度）。");
        }
