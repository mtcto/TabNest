...
> 评审结论：建议立项，但必须先完成窗口承载技术 Spike；MVP 不应直接承诺“任何应用都能像 Groupy 一样完美嵌入”。
...
3. **不破坏原应用**：不做进程注入，不修改目标应用文件，不模拟应用内部标签。
...
Groupy 2 的内部实现未公开。基于其公开行为和更新历史，可以做出以下工程推断：


- 它至少需要跨进程观察顶层窗口的创建、显示、移动、激活、销毁和标题变化；
- 它需要在窗口标题栏或标题栏上方绘制/托管标签区域；
- 它需要协调窗口位置、最大化、恢复、任务栏、虚拟桌面和 DPI；
- 它需要处理 Windows 11 File Explorer/Notepad 原生标签和第三方自定义标题栏；
- 它可能使用窗口样式调整、窗口重排、消息/事件 hook、DWM 能力以及针对特定应用的兼容逻辑；
- 不能仅凭公开资料确认它是否以 `SetParent` 作为主要实现，也不能将“窗口嵌入”与“标题栏整合”简单等同。
...
| 层 | 选择 | 结论 |
|---|---|---|
| 语言 | C# | 保留，适合 1～2 人快速迭代和 AI 辅助 |
| UI | WinUI 3 | 保留，用于现代 Fluent UI、设置中心和标签轨道 |
| 平台 | Windows App SDK Stable | 保留，只使用稳定通道，发布前锁定版本 |
| 窗口底层 | Win32 API + P/Invoke | 必选，WinUI 3 不会替代外部 HWND 管理 |
| 架构 | MVVM + 领域服务 + Interop 边界 | 保留，但避免过度拆分微型项目 |
| 数据 | 本地版本化 JSON | MVP 足够；后续可迁移 SQLite |
| 部署 | MVP 优先 MSIX/打包测试；企业和离线场景支持自包含安装 | 根据窗口权限和部署反馈调整 |
| 更新 | 版本检查 + 安全回滚 | MVP 只做人工/安装器更新入口，自动升级后置 |
...
- 不是真正的单窗口，任务栏和 Alt+Tab 可能仍看到多个成员；
...
- P0 只提供“标签在标题栏上方/集成承载的产品模式选择”这一层概念，不在首版暴露过多皮肤参数；
...
|---|---:|---:|---:|---|---|
| 跨进程 SetParent 导致应用异常 | 高 | 高 | P0 | MVP 默认虚拟聚合；真实嵌入只对白名单启用；保存快照并可拆分 | 无已知不可恢复案例 |
| 外部窗口关闭/崩溃后 ghost Tab | 高 | 中 | P0 | 监听 DESTROY，标记 Closed，提供清理和恢复 | 不出现永久幽灵标签 |
| 焦点落到 TabNest 而非目标应用 | 中 | 高 | P0 | 激活顺序、消息队列、焦点验证和重试 | 目标用例切换成功率 ≥95% |
...
| 方案 | 优点 | 缺点 | 适合阶段 |
|---|---|---|---|
| WinUI 3 + 虚拟聚合 | UI 现代，开发快，风险可控 | 不是物理单窗口，任务栏身份可能保留 | MVP 首选 |
...
- 无活动窗口事件时，TabNest 进程平均 CPU < 1%；
- 10 个普通标签、无缩略图常驻时，TabNest 进程工作集软目标 < 180MB；
- 目标应用正常响应时，标签切换 P95 ≤ 150ms；
- 窗口事件到 UI 状态更新 P95 ≤ 200ms；
- 事件风暴不会无限增长队列；
- 运行 8 小时不出现 hook 泄漏、幽灵标签或持续增长的句柄数。
...
- 兼容性诊断中心；
- 对 IntelliJ、VS Code、Chrome、Edge 建立白名单实验性嵌入。
...
1. 保留 C# + WinUI 3 + Windows App SDK，不默认更换技术栈。
2. 采用 .NET 10 LTS 为主选，若 WinUI tooling 不稳定则回退到锁定的 .NET 8 LTS。
3. MVP 使用虚拟窗口聚合，真实嵌入作为白名单实验能力。
4. MVP 只承诺 Windows 11 x64 标准桌面应用的支持矩阵。
5. P0 以手动分组、切换、拖拽、拆分、恢复和诊断为核心。
6. 自动分组、工作区、多显示器增强、插件和云同步后置。
